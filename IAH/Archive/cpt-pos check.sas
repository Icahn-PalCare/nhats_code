...
data both_beneid (keep= bene_id wave);
set bothclaims;
run;

proc sql;
create table both_pos as select a.*, b.*
from both_beneid a
inner join posclaims b
on a.bene_id=b.bene_id and a.wave=b.wave;
quit;

proc sort data=both_pos nodupkey; by bene_id clm_id hcpcs_cd; run;
proc freq data=both_pos order=freq; tables hcpcs_cd; run;

 
proc sort data=both_pos nodupkey; by bene_id wave; run;
