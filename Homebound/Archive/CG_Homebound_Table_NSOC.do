...
local rn : word count `cvars' `ivars' n n n n 
...
foreach i in 1 2 3 4 {
 	sum c1relatnshp`weight' if homebound_cat1==`i'
		mat test[`r'+2,`c']=r(N)
		mat test[`r'+3,`c']=r(sum_w)
		local c=`c'+3
}
restore


mat rownames test=`cvars' `ivars' "Sample N" "Sample N Weighted" "Sample Unique SP" "Sample Unique SP Weighted"
frmttable using "E:\nhats\data\Projects\Homebound\logs\CG_Homebound_Table_NSOC_2_11", statmat(test) ctitles("" "Mean/N Homebound" "SD/%" "" "Mean/N Semi (Never by Self)" "SD/%" "P-value" "Mean/N Semi: Needs Help/Diffculty" "SD/%" "P-Value" "Mean/N Not Homebound" "SD/%" "P-value") ///
store(test) sdec(2) replace varlabels  title("Sample Characteristics by 1 year Death Weighted (Full NSOC Sample)")
}
