...
local c=4
foreach i in 0 1 {
 	sum c1relatnshp`weight' if died==`i'
		mat test[`r',`c']=r(N)
...
mat rownames test=`cvars' `ivars' "Sample N" 
frmttable using "E:\nhats\data\Projects\Caregivers\logs\demo_nonltcs_noprim(12_16)_weights", statmat(test) ctitles("" "Mean/N All" "SD/%" "Missing" "Mean/N Survivors" "SD/%" "Mean/N Decedents" "SD/%" "P-value") ///
store(test) sdec(2,2,2,2,2,2,2,3) replace varlabels  title("Sample Characteristics by 1 year Death Weighted (Full NSOC Sample)")
}
