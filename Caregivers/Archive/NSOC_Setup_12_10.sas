...
run;
option nospool;
%let varlist = che1hrtattck che1othheart che1arthrits che1diabetes che1lungdis che1cancer che1pain che1brethprb che1lowenrgy che1sleeptrb che1lost10lb che1highbld che1osteoprs che1seeing che1hearing;
%let varlist1 = ever_heart_attack ever_heart_disease ever_arthritis ever_diabetes ever_lung_dis ever_cancer ever_pain ever_breath_prob ever_low_enrgy trb_back_sleep lost_10_lbs_yr ever_high_bp
...
%let varlist = che1health che1painlmt che1brethlmt che1enrgylmt cca1hwoftpc cca1hwofthom cca1hwoftshp cca1hwoftchs;
%let varlist1 =  self_health pain_lim_act breath_lim_act enrgy_lim_act help_personal_care help_get_around help_shopping help_chores;
%macro help1;
%let i = 1;
%let var = %scan(&varlist,&i);
%let var1 = %scan(&varlist1, &i);
...
ods rtf body = "E:\nhats\projects\Caregivers_KO\11242015\diffculty_levels.rtf";
%macro difficulty1;
%let i = 1;
%let var = %scan(&varlist,&i);
%let var1 = %scan(&varlist1, &i);
...
proc export data=raw_op_sens1 outfile = "E:\nhats\data\Projects\Caregivers\raw_op_sens2.dta" dbms = dta replace;
run;
...
proc export data=nsoc_w_age outfile = "E:\nhats\data\Projects\Caregivers\nsoc_age.dta" dbms = dta replace;
run;
data nsoc1;
set nsoc_w_age;
run;
...
if cec1hrsweek = 997 then cec1hrsweek = cec1hrslstwk;
...
if all_helpers = 1 and (op1paidhelpr = -1|op1paidhelpr = 2) then helper_unpaid = 1;
if all_helpers = 1 and (op1paidhelpr = 1) then helper_unpaid = 0;
/*helper family CHANGED in 12/10/2015 from being actual family to indication of them being unpaid. */
all_helper_family = .;
if all_helpers = 1 and helper_unpaid = 1 then all_helper_family = 1;
if all_helper_family ~= 1 then all_helper_family = 0;
run;
proc freq data=raw_op_inf_cg1;
table all_helpers all_helper_family helper_unpaid ;
run;
/*
data test;
...
run;
data test1;
set raw_op_inf_cg2 (keep = spid opid nsoc_eligible helper_unpaid informal_nsoc_comp informal_nsoc_comp_i nsoc_eligible_i op1ishelper all_helpers informal_nsoc_elig all_helper_tot all_helper_family_tot);
run;
data raw_op_inf_cg3;
set raw_op_inf_cg2;
by spid opid;
...
run;
data test3;
set raw_op_inf_cg3 (keep = spid opid nsoc_eligible helper_unpaid informal_nsoc_comp informal_nsoc_comp_i nsoc_eligible_i op1ishelper all_helpers informal_nsoc_elig all_helper_tot all_helper_family_tot );
run;
...
solo_cg_fam = .;
if helper_unpaid = 1 and all_helper_family_tot = 1 then solo_cg_fam = 1;
if helper_unpaid = 1 and all_helper_family_tot > 1 then solo_cg_fam = 0;
if helper_unpaid = 0 and all_helper_family_tot = 0 then solo_cg_fam = 1;
if helper_unpaid = 0 and all_helper_family_tot > 0 then solo_cg_fam = 0;
run;

proc freq data=nsoc_helpers1;
table helper_unpaid is_imm_family solo_cg solo_cg_fam ;
run;
proc freq data=nsoc_helpers1;
where helper_unpaid = 1;
table all_helper_family_tot;
run;
proc freq data=nsoc_helpers1;
...
proc freq data=raw_op;
table op1prsninhh op1relatnshp op1gender op1childinhh op1chnotinhh;
run;
proc freq data=raw_op;
where op1chnotinhh = 1;
table op1prsninhh; 
run;
proc freq data=raw_nhats_r1;
table hh1livwthspo hh1proxlivsp hh1othlvhere hh1dlvngarrg hh1dhshldnum;
run;
proc freq data=raw_nhats_r1;
where hh1dhshldnum = 1;
table hh1dlvngarrg;
run;
data op_in_hh;
set raw_op (keep = spid opid op1relatnshp op1prsninhh op1childinhh op1chnotinhh);
in_hh = op1prsninhh;
if in_hh < 0 then in_hh = .;
if in_hh = . and op1childinhh > 0 then in_hh = op1childinhh;
if in_hh = . and op1chnotinhh = 1 then in_hh = 2;
run;
proc freq data=op_in_hh;
table in_hh;
run;

/*These are variables that are already translated into the OP from the SP. 
data test ;
set raw_nhats_r1 (keep = spid is1dproxyid hh1dspouseid hh1livwthspo hh1proxlivsp hh1othlvhere hh1dlvngarrg hh1dhshldnum );
run;
proc sql;
create table test1
as select a.*, b.hh1livwthspo, c.hh1proxlivsp
from raw_op a
...
quit;
proc freq data=test1;
table op1prsninhh hh1livwthspo hh1proxlivsp;
run;
data test2;
set test1 (keep = spid opid op1prsninhh op1relatnshp hh1livwthspo hh1proxlivsp);
if op1prsninhh < 1 then op1prsninhh = .;
if hh1livwthspo < 1 then hh1livwthspo = .;
if hh1proxlivsp < 1 then hh1proxlivsp = .;

nhats_hh = op1prsninhh;
if nhats_hh = -1 and hh1proxlivsp ~= . then nhats_hh = hh1proxlivsp;
if nhats_hh = -1 and hh1livwthspo ~= . then nhats_hh = hh1livwthspo;
run;
...
data nsoc1;
set nsoc1;
drop hh1dlvngarrg hh1dhshldnum op1prsninhh hh1livwthspo;
run;

proc sql;
create table nsoc_w_hh
as select a.*, f.in_hh, b.op1prsninhh, c.hh1dlvngarrg, c.hh1dhshldnum, d.hh1livwthspo, e.hh1proxlivsp, c.hh1othlvhere
from nsoc1 a
left join raw_op b
on a.spid = b.spid and a.opid = b.opid
left join op_in_hh f
on a.spid = f.spid and a.opid = f.opid
left join raw_nhats_r1 c
on a.spid = c.spid
left join raw_nhats_r1 d
on a.spid = d.spid and a.opid=d.hh1dspouseid
left join raw_nhats_r1 e
on a.spid = e.spid and a.opid=e.is1dproxyid;
quit;

data test;
set nsoc_w_hh (keep = spid opid in_hh chd1numinhh op1prsninhh hh1dlvngarrg hh1dhshldnum hh1livwthspo hh1proxlivsp hh1othlvhere);
run;
proc freq data=test;
table hh1othlvhere;
run;
proc freq data=nsoc_w_hh;
where hh1dlvngarrg = 1;
table op1prsninhh;
run;
data nsoc_w_hh1;
set nsoc_w_hh;
if cg_spouse = 0 then hh1livwthspo = .;
/*sp lives alone variable*/
sp_lives_alone = 0;
if hh1dlvngarrg = 1 then sp_lives_alone = 1;
/*if SP lives alone then they have no other person in HH*/
lives_in_hh = .;
if op1prsninhh = 1 then lives_in_hh = 1;
if op1prsninhh = 2 then lives_in_hh = 0;
if lives_in_hh = . and sp_lives_alone = 1 then lives_in_hh = 0;
/*derived variable from children not being in hh*/
if lives_in_hh = . and in_hh = 1 then lives_in_hh = 1;
if lives_in_hh = . and in_hh = 2 then lives_in_hh = 0;
/*If not spouse and if the person said they live with only spouse then this person does not live in HH?*/
if lives_in_hh = . and cg_spouse = 0 and hh1dlvngarrg = 2 then lives_in_hh = 0;
run;
proc freq data=nsoc_w_hh1;
table sp_lives_alone op1prsninhh lives_in_hh;
run;
proc freq data=nsoc_w_hh1;
where lives_in_hh=.;
table c1relatnshp;
...
proc freq data=nsoc_w_hh1;
table cg_spouse chd1martstat;
run;
proc freq data=nsoc_w_hh1;
where cg_spouse = 0;

table chd1numinhh / missing;
run;
