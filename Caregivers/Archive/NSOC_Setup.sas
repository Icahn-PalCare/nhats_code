...
proc freq data=raw_op;
table op1prsninhh op1relatnshp op1gender ;
run;
proc freq data=raw_nhats_r1;
table hh1livwthspo hh1proxlivsp hh1othlvhere;
run;
data test ;
set raw_nhats_r1 (keep = spid is1dproxyid hh1: );
run;
proc sql;
create table test1
...
nhats_hh = hh1livwthspo;
if nhats_hh = . then nhats_hh = hh1proxlivsp;
if nhats_hh = . then nhats_hh = 999;
run;
proc freq data=test2;
table op1prsninhh*nhats_hh;
run;
...
as select a.*, b.op1prsninhh
from nsoc1 a
left join raw_op b
on a.spid = b.spid and a.opid = b.opid;
quit;
proc freq data=nsoc_w_hh;
table op1prsninhh*chd1numinhh op1prsninhh*cca1hwget2sp;
run;
data nsoc_w_hh;
set nsoc_w_hh;
...
/*Is immediate family*/
is_imm_family = .;
if op1relatnshp >=2 and op1relatnshp <= 4 then is_imm_family = 1;
if is_imm_family ~= 1 then is_imm_family = 0;
/*helper family*/
all_helper_family = .;
if all_helpers = 1 and is_family = 1 then all_helper_family = 1;
if all_helper_family ~= 1 then all_helper_family = 0;
/*helper immediate family*/
all_helper_imm_family = .;
if all_helpers = 1 and is_imm_family = 1 then all_helper_imm_family = 1;
if all_helper_imm_family ~= 1 then all_helper_imm_family = 0;
run;
...
set raw_op_inf_cg1;
by spid opid;
retain informal_nsoc_elig all_helper_tot all_helper_family_tot all_helper_imm_family_tot;
...
all_helper_imm_family_tot = all_helper_imm_family_tot + all_helper_imm_family;
if first.spid then do;
informal_nsoc_elig = nsoc_eligible_i;
all_helper_tot=all_helpers;
all_helper_family_tot = all_helper_family;
all_helper_imm_family_tot = all_helper_imm_family;
end;
run;

data test1;
set raw_op_inf_cg2 (keep = spid opid nsoc_eligible is_family is_imm_family informal_nsoc_comp informal_nsoc_comp_i nsoc_eligible_i op1ishelper all_helpers informal_nsoc_elig all_helper_tot all_helper_family_tot all_helper_imm_family_tot);
run;
data raw_op_inf_cg3;
set raw_op_inf_cg2;
by spid opid;
...
run;
data test3;
set raw_op_inf_cg3 (keep = spid opid nsoc_eligible is_family is_imm_family informal_nsoc_comp informal_nsoc_comp_i nsoc_eligible_i op1ishelper all_helpers informal_nsoc_elig all_helper_tot all_helper_family_tot all_helper_imm_family_tot);
run;
ods rtf body = "E:\nhats\data\Projects\Caregivers\logs\Total_Helper.rtf";
proc freq data=raw_op_inf_cg3;
table informal_nsoc_comp  all_helper_tot diff_helper all_helper_family_tot all_helper_imm_family_tot;
run;
ods rtf close;

proc sql;
create table nsoc_helpers
as select a.*, b.informal_nsoc_comp, b.informal_nsoc_elig, b.all_helper_tot, b.all_helper_family_tot, b.all_helper_imm_family_tot
from nsoc1 a 
...
table informal_nsoc_comp informal_nsoc_elig all_helper_tot all_helper_family_tot all_helper_imm_family_tot;
run;
...
/*creating other family variables with NSOC*/
is_family = .;
if (c1relatnshp >=1 and c1relatnshp <= 29 )|c1relatnshp=91 then is_family = 1;
if is_family ~= 1 then is_family = 0;
/*Is immediate family*/
is_imm_family = .;
if c1relatnshp >=2 and c1relatnshp <= 4 then is_imm_family = 1;
if is_imm_family ~= 1 then is_imm_family = 0;
/*No Other Family Member*/
solo_cg_fam = .;
if is_family = 1 and all_helper_family_tot = 1 then solo_cg_fam = 1;
if is_family = 1 and all_helper_family_tot > 1 then solo_cg_fam = 0;
if is_family = 0 and all_helper_family_tot = 0 then solo_cg_fam = 1;
if is_family = 0 and all_helper_family_tot > 0 then solo_cg_fam = 0;
/*No Other Immediate Family Member*/
solo_cg_imm_fam = .;
if is_imm_family = 1 and all_helper_imm_family_tot = 1 then solo_cg_imm_fam = 1;
if is_imm_family = 1 and all_helper_imm_family_tot > 1 then solo_cg_imm_fam = 0;
if is_imm_family = 0 and all_helper_imm_family_tot = 0 then solo_cg_imm_fam = 1;
if is_imm_family = 0 and all_helper_imm_family_tot > 0 then solo_cg_imm_fam = 0;
run;

proc freq data=nsoc_helpers1;
table is_family is_imm_family solo_cg solo_cg_fam solo_cg_imm_fam;
run;
proc freq data=nsoc_helpers1;
where is_family = 1;
...
table solo_cg_imm_fam all_helper_imm_family_tot ;
run;
proc freq data=nsoc_helpers2;
where solo_cg_imm_fam = 1;
table all_helper_tot;
run;
proc freq data=nsoc_helpers2;
table solo_cg_fam  all_helper_family_tot;
run;
proc freq data=nsoc_helpers2;
where solo_cg_fam = 1;
table all_helper_tot;
