...
merge 1:1 spid opid using "E:\nhats\data\NHATS Sensitive\r1_sensitive\NSOC_Round_1_File_v2.dta", ///
keepusing(w1cgfinwgt0) gen(merge)

svyset [pw=w1anfinwgt0]
keep if op1ishelp==1
gen nsoc_ivw=w1cgfinwgt0!=.
by spid, sort: egen nsoc_sample=max(nsoc_ivw)
keep if nsoc_sample==1
drop if died==.
gen tot=1 
gen lived=died==0
...
frmttable using "E:\nhats\data\Projects\Caregivers\logs\Helper_Table_nsoc_sample", ///
...
title("SPs and their Help, all NSOC-participating SPs, by SP survivial status") ///
note("restricted to 1350 SPs with at least one caregiver in the NSOC and known survival status")
...
\"Laundry"\"Shopping"\"Meals"\"Banking"\"Medicine"\""\"Any ADL"\"Any IADL") ///
ctitles("" "Total" "" "Survivors" "" "Decedents" "" \ "" "Mean/N Total" "SD/%" ///
"Mean/N Survivors" "SD/%" "Mean/N Decedents" "SD/%" ) ///
store(test1) sdec(2,2,2,2,2,2,2,4) addtable ///
title("Helpers, all NSOC-participating SPs, by SP survivial status") ///
note("restricted to helpers of 1350 SPs with at least one caregiver in the NSOC and known survival status")
